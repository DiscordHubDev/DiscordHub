...
export default function CategorySearch({ categories, onCategoryChange, onCustomCategoryAdd }: CategorySearchProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        />
        <Button size="icon" onClick={handleAddCustomCategory} className="bg-[#5865f2] hover:bg-[#4752c4]">
          <Plus size={16} />
        </Button>
      </div>

      {/* 顯示過濾後的分類 */}
      {filteredCategories.length === 0 && <p className="text-gray-400 text-sm">沒有找到相符的分類</p>}
    </div>
  )
}
