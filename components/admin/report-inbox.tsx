...
import { ResolveDialog } from '../ReportResolveDialog';
...
export default function ReportInbox({ reports: allReports }: ReportProps) {
    ...
    const [selectedReport, setSelectedReport] =
        useState<ReportInBoxType | null>(null);
    ...
    const handleStatusChange = async (
        reportId: string,
        newStatus: keyof typeof REPORT_STATUS_ENUM,
    ) => {
        const statusEnum = REPORT_STATUS_ENUM[newStatus];

        setDialogInfo({ reportId: reportId, status: statusEnum });

        const res = await updateReport({ reportId, status: statusEnum });
    ...
    const handleSeverityChange = async (
        reportId: string,
        newSeverity: keyof typeof REPORT_SEVERITY_ENUM,
    ) => {
        const severityEnum = REPORT_SEVERITY_ENUM[newSeverity];

        const res = await updateReport({ reportId, severity: severityEnum });
    ...
    const filteredReports = useMemo(() => {
        ...
        return reports.filter((report) => {
            const matchesSearch =
                ...
                getSeverityInfo(report.severity)
                    .label.toLowerCase()
                    .includes(search);
    ...
    return (
        <Card className="bg-[#2F3136] border-[#202225] text-white">
          ...
          `}
                                ...
                                {/* 右側：處理資訊 */}
                                {selectedReport.status !== 'pending' && (
                                    <div className="text-gray-400 text-xs sm:text-sm flex flex-col justify-start min-w-[200px] text-left ">
