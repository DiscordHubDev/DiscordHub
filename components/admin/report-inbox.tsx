...
export default function ReportInbox({ reports: allReports }: ReportProps) {
  ...
  return (
    <Card className="bg-[#2F3136] border-[#202225] text-white">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {filteredReports.length === 0 ? (
              <div className="text-center py-6 text-gray-400">沒有任何檢舉</div>
            ) : (
                ...
                {selectedReport.status !== 'pending' && (
                  <div className="text-gray-400 text-xs sm:text-sm flex flex-col justify-start min-w-[200px] text-left ">
                    <p className="break-words">
                      此檢舉由{' '}
                      <strong>
                        {selectedReport.handledBy?.username || '未知管理員'}
                      </strong>{' '}
                      已於{' '}
                      <strong>
              ...
              {selectedReport.status === 'pending' && (
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleStatusChange(selectedReport, 'rejected')
                    }
                  >
      ...
      {dialogInfo && (
        <ResolveDialog report={dialogInfo.report} status={dialogInfo.status} />
      )}
    </Card>
  );
}
